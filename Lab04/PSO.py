...
class Particle:
    ...
    def update_position(self):
        ...
        if self.x > x_max:
            offset = self.x - 10.0
            self.x = abs(x_max) - offset
        if self.x < (-1) * x_max:
            offset = abs(self.x) - 10.0
            self.x = (-1) * x_max + offset
        if self.y > y_max:
            offset = self.y - 10.0
            self.y = abs(y_max) - offset
        if self.y < (-1) * y_max:
